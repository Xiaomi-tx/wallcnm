...
const uploasdMiddle = async (ctx, next) => {
  let err = await upload.array('photo')(ctx, next)
        .then(res => {
        })
        .catch(err => {
          // console.log(err);
          ctx.body = new Error({
            status: 406,
            message: "文件过大！！！而且文件必须是 jpg gif jpeg png 格式！还有一次只能上传3张"
          })
          return;
        })
}
